...
------------------------------
-- LOCAL MODIFICATION (Kiro): upstream Hive has
--     \i hive-txn-schema-2.3.0.postgres.sql;
-- here. psql resolves \i relative to its working directory, and the Postgres
-- image's entrypoint runs these scripts as `psql -f /docker-entrypoint-initdb.d/*.sql`
-- from /, so the include would fail with "No such file or directory".
-- Instead the txn/lock tables live in 02-hive-txn-schema.sql and the VERSION
-- row (which upstream inserts last, after the include) is in 03-hive-version.sql.
-- The Postgres entrypoint runs *.sql in lexical order, so 01 -> 02 -> 03
-- reproduces upstream's ordering exactly.
